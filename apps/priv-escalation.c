...
/* offsets for cred */
#define OFFSET_TASK_CRED 2944
#define OFFSET_UID_CRED  4
#define OFFSET_GID_CRED  8
		     ...
		     struct task_struct *t)
{
    ...
    /* Check credentials */
    t->vcred = vhead + OFFSET_TASK_CRED;
    t->pcred = __phys_addr(t->vcred);

    ret = dma_read(nt, t->pcred, &t->cred, sizeof(t->cred));
    if (ret < sizeof(t->cred)) 
        return -1;

    check_task_value(t, cred);
    uintptr_t p = __phys_addr(t->cred);
    ret = dma_read(nt, p + OFFSET_UID_CRED, &t->uid, sizeof(t->uid));
    if (ret < sizeof(t->uid)) {
		return -1;
	}
    ret = dma_read(nt, p + OFFSET_GID_CRED, &t->gid, sizeof(t->gid));
    if (ret < sizeof(t->gid)) {
        return -1;
	}
...
{
#ifdef PRINT_OUT
	printf("PhyAddr             PID STAT  COMMAND          | uid gid\n");
#endif
}
...
int print_task_struct(struct nettlp *nt, struct task_struct t)
{
	...
	// printf("ppid: 0x%lx, pcomm: 0x%lx, pstate: 0x%lx\n", t.ppid, t.pcomm, t.pstate); // debug
...
#ifdef PRINT_OUT
	printf("%#016lx %6d %c    %16s  %4d %4d\n",
	       t.phead, pid, state_to_char(state), comm, t.uid, t.gid);
#endif
...
int task(struct nettlp *nt, uintptr_t vhead, uintptr_t parent)
{
	...
	if (t.children_next != t.vchildren) {
		...
		// printf("CHILDREN: children_next: %llx, vchildren: %llx\n", t.children_next, t.vchildren);
